<template>
  <view class="v-screen visas-screen">
    <VNavBar :title="country.name || countryName || '签证列表'" :show-back="true" />
    ...
    <view class="v-page-width visas-body">
      <text class="v-section-title">可选签证类型</text>

      <view class="visa-stack">
        <view v-if="loading" class="v-card visa-card">
        ...
        <view v-if="!loading && !visaOptions.length" class="v-card visa-card">
        ...
        <view class="drawer-body">
          <view class="drawer-head">
            <view class="drawer-icon">
              <VIcon class="drawer-icon-inner" :name="activeVisa.visaFree ? 'flag-o' : 'bookmark'" />
            </view>
          ...
          <scroll-view v-else scroll-y class="drawer-scroll">
            <view class="drawer-block">
              <view class="badge-row">
                <VTag :text="activeVisa.visaType || '签证类型'" type="primary" />
                <VTag v-if="activeVisa.visaFree" text="免签" type="warning" />
                <VTag :text="activeVisa.entries || '入境次数待定'" type="default" />
              </view>

              <view class="drawer-note visa-drawer-intro">
                <VIcon class="drawer-line-icon" name="notes-o" />
                <text>{{ activeVisa.longIntro || activeVisa.description }}</text>
              </view>

              <view class="detail-grid">
                <view class="detail-box">
                  <VIcon class="detail-box-icon" name="clock" />
                  <text class="detail-box-label">办理周期</text>
                  <text class="detail-box-value">{{ activeVisa.processingTime || '-' }}</text>
                </view>
                <view class="detail-box">
                  <VIcon class="detail-box-icon" name="cash" />
                  <text class="detail-box-label">预估费用</text>
                  <text class="detail-box-value">{{ activeVisa.fee || '-' }}</text>
                </view>
                <view class="detail-box">
                  <VIcon class="detail-box-icon" name="bookmark" />
                  <text class="detail-box-label">有效期</text>
                  <text class="detail-box-value">{{ activeVisa.validity || '-' }}</text>
                </view>
                <view class="detail-box">
                  <VIcon class="detail-box-icon" name="card" />
            ...
            <view v-if="activeVisa.steps && activeVisa.steps.length" class="drawer-block">
              ...
              <view
                ...
                class="guide-card process-card"
              >
                <view class="guide-title-row">
                  <view class="guide-index">{{ idx + 1 }}</view>
                  <text class="guide-title process-card-title">{{ step.title }}</text>
                </view>
                <text class="guide-desc process-card-desc">
                  {{ step.desc || (step.strategies || []).join('；') || '查看详情后再准备下一步。' }}
                </text>
              </view>
            </view>
            ...
            <view
              class="join-btn"
              :class="{ 'join-btn--disabled': creatingGoal }"
              @tap="addToGoal"
            >
