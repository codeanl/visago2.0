const GOALS_KEY = 'visanow_vant_goals'
...
const GOAL_STEP_TEMPLATES = [
  {
    ...
    title: '申请',
    tasks: [
      { id: 'confirm-visa', title: '确认签证方案', icon: 'search' },
    ],
  },
  {
    stepKey: 'docs',
    title: '材料',
    tasks: [
      { id: 'prepare-materials', title: '整理基础材料', icon: 'notes-o' },
      { id: 'note-plan', title: '补充办理备注', icon: 'edit' },
    ],
  },
  {
    stepKey: 'book',
    title: '预约',
    tasks: [
      { id: 'schedule-time', title: '安排办理时间', icon: 'clock' },
    ],
  },
  {
    stepKey: 'result',
    title: '结果',
    tasks: [
      { id: 'mark-complete', title: '记录本次结果', icon: 'passed' },
    ],
  },
]
...
function normalizeCountryCode(countryName, countryFlag) {
  const flag = String(countryFlag || '').trim()
  const upperFlag = flag.toUpperCase()
  if (CODE_TO_FLAG_EMOJI[upperFlag]) return upperFlag === 'UK' ? 'GB' : upperFlag
  if (FLAG_EMOJI_TO_CODE[flag]) return FLAG_EMOJI_TO_CODE[flag]

  const name = String(countryName || '').trim()
  if (COUNTRY_NAME_TO_CODE[name]) return COUNTRY_NAME_TO_CODE[name]
  const matchedName = Object.keys(COUNTRY_NAME_TO_CODE).find((item) => name.includes(item))
  if (matchedName) return COUNTRY_NAME_TO_CODE[matchedName]

  const alphaCode = upperFlag.replace(/[^A-Z]/g, '')
  if (alphaCode.length === 2) return alphaCode === 'UK' ? 'GB' : alphaCode
  return ''
}

function buildCountryFlag(goal) {
  const code = normalizeCountryCode(goal.countryName, goal.countryFlag)
  return {
    code,
    text: CODE_TO_FLAG_EMOJI[code] || goal.countryFlag || (goal.countryName ? String(goal.countryName).slice(0, 1) : '签'),
  }
}
...
export function updateGoalNote(visaId, note) {
  ...
  items[index] = normalizeGoal({
    ...
    taskStateMap: {
      ...
      note_plan: Boolean(String(note || '').trim()),
    },
...
export function toggleGoalTask(visaId, taskId) {
  ...
  if (index < 0) return items
  const current = items[index]
  if ((current.resultStatus || 'pending') !== 'pending') {
    return items
  }
...
export function buildGoalView(rawGoal) {
  ...
  const countryFlag = buildCountryFlag(goal)
  ...
  return {
    ...
    countryFlag: countryFlag.text,
    countryFlagCode: countryFlag.code,
    progress,
    steps,
    tips,
